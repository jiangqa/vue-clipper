<template>
  <div class="clipper-container" ref="clipper">
    ...
    <div class="action-bar">
      ...
      <button class="btn-change" @click="_change">
        旋转
      </button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        nowRotate:0,
        changeRotate:90,
        clipperClientRectW:0,
        clipperClientRectH:0,
        clipperWidth:0,
        clipperHeight:0
      }
    },
    ...
    methods: {
      ...
      _initCanvas() {
        ...
        //因为canvas在手机上会被放大，因此里面的内容会模糊，这里根据手机的devicePixelRatio来放大canvas，
        // 然后再通过设置css来收缩，因此关于canvas的所有值或坐标都要乘以devicePixelRatio
        this.clipperClientRectW = clipperClientRect.width;
        this.clipperClientRectH = clipperClientRect.width;
        this.clipperWidth = clipperWidth;
        this.clipperHeight = clipperHeight;
        $canvas.style.width = clipperClientRect.width + 'px';
        $canvas.style.height = clipperClientRect.width + 'px';
        $canvas.width = this._ratio(clipperClientRect.width);
        $canvas.height = this._ratio(clipperClientRect.width);
      ...
      _initEvent() {
        ...
        $gesture.addEventListener('touchmove', e => {
          ...
          if (e.touches.length === 1) {
            ...
            this._drawImage(renderX, renderY, this.imgCurrentWidth, this.imgCurrentHeight);
          } else if (e.touches.length === 2) {
            let finger1 = e.touches[0],
              finger2 = e.touches[1],
              f1x = finger1.pageX - cClientRect.left,
...
//                        this._drawImage(x/2, y/2, this.imgCurrentWidth, this.imgCurrentHeight);
//                    }
//
        });
      },
      ...
      _initImg(w, h) {
        ...
        if (eW <= maxW && eH <= maxH) {
          ...
          console.log(eH,eW)
          this._drawImage(-eW/2,-eH/2, eW, eH);
        } else {
          this._initImg(eW, eH);
        }
      },
      _drawImage(x, y, w, h) {
        ...
        //更新canvas
        console.log(this._ratio(this.clipperWidth)/2,this._ratio(this.clipperHeight)/2)
        this.ctx.save()
        this.ctx.translate(this._ratio(this.clipperClientRectW)/2, this._ratio(this.clipperClientRectH)/2)
        this.ctx.rotate(this.nowRotate*Math.PI/180)
        this.ctx.drawImage(this.$img, this._ratio(x), this._ratio(y), this._ratio(w), this._ratio(h));
        this.ctx.restore()
        this.pCtx.save()
        this.pCtx.translate(this._ratio(this.clipperWidth)/2,this._ratio(this.clipperHeight)/2)
        this.pCtx.rotate(this.nowRotate*Math.PI/180)
        console.log( this._ratio(x), this._ratio(y))
        this.pCtx.drawImage(this.$img, this._ratio(x), this._ratio(y), this._ratio(w), this._ratio(h));
        this.pCtx.restore()
      ...
      _change(){
        ...
        this.nowRotate +=this.changeRotate
        if(this.nowRotate == 360){
          this.nowRotate = 0
        }
        this._drawImage(this.imgX,this.imgY,this.imgCurrentWidth,this.imgCurrentHeight)
      }
    }
  }
</script>
